...
<?php
	...
	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ), _x( 'Email', 'share to', 'sharedaddy' ), __( 'Click to email this to a friend', 'sharedaddy' ), 'share=email' );
	}
...
?>
	<div id="sharing_email" style="<?php if ( $visible === false ) echo 'display: none;'; ?>">
		<form action="" method="post">
			<label for="target_email"><?php _e( 'Send to Email Address', 'sharedaddy' ) ?></label>
			...
			<?php else : ?>
				...
				<label for="source_name"><?php _e( 'Your Name', 'sharedaddy' ) ?></label>
				<input type="text" name="source_name" id="source_name" value="" />
				
				<label for="source_email"><?php _e( 'Your Email Address', 'sharedaddy' ) ?></label>
			...
			<button class="sharing_cancel btn btn-info btn" type="button" ><?php _e( 'Cancel', 'sharedaddy' ); ?></button>
			...
			<div class="errors errors-2" style="display: none;">
				<?php _e( 'Email check failed, please try again', 'sharedaddy' ); ?>
			</div>
			
			<div class="errors errors-3" style="display: none;">
				<?php _e( 'Sorry, your blog cannot share posts by email.', 'sharedaddy' ); ?>
			</div>
		</form>
	</div>
<?php
...
class Share_Stumbleupon extends Sharing_Advanced_Source {
	...
	public function get_name() {
		return __( 'StumbleUpon', 'sharedaddy' );
	}
	...
	public function get_display( $post ) {
		if ( $this->smart == 'smart' )
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'StumbleUpon', 'share to', 'sharedaddy' ), __( 'Click to share on StumbleUpon', 'sharedaddy' ), 'share=stumbleupon' );
	}		
...
class Share_Reddit extends Sharing_Advanced_Source {
	...
	public function get_display( $post ) {
		if ( $this->smart == 'smart' )
			...
			return $this->get_link( get_permalink( $post->ID ), __( 'Reddit', 'share to', 'sharedaddy' ), __( 'Click to share on Reddit', 'sharedaddy' ), 'share=reddit' );
	}	
...
class Share_Digg extends Sharing_Advanced_Source {
	...
	public function get_display( $post ) {
		if ( $this->smart ) {
			$url = $this->get_link( 'http://digg.com/submit?url='. urlencode( apply_filters( 'sharing_permalink', get_permalink( $post->ID ), $post->ID, $this->id ) ) . '&amp;title=' . urlencode( $post->post_title ), 'Digg', __( 'Click to Digg this post', 'sharedaddy' ) );
			return '<div class="digg_button">' . str_replace( 'class="', 'class="DiggThisButton DiggCompact ', $url ) . '</div>';
		}
		else
			return $this->get_link( get_permalink( $post->ID ), _x( 'Digg', 'share to', 'sharedaddy' ), __( 'Click to Digg this post', 'sharedaddy' ), 'share=digg' );
	}	
...
class Share_Facebook extends Sharing_Advanced_Source {
	...
	public function get_display( $post ) {
		if ( $this->share_type == 'share' )
			return '<div class="facebook_button"><a name="fb_share" rel="nofollow" type="button" share_url="' . get_permalink( $post->ID ) . '" href="http://www.facebook.com/sharer.php?u=' . rawurlencode( get_permalink( $post->ID ) ) . '&t=' . rawurlencode( $post->post_title ) . '">'._x( 'Share', 'on facebook', 'sharedaddy' ).'</a><script src="http://static.ak.fbcdn.net/connect.php/js/FB.Share" type="text/javascript"></script></div>';
		...
		return $this->get_link( get_permalink( $post->ID ), _x( 'Facebook', 'share to', 'sharedaddy' ), __( 'Share on Facebook', 'sharedaddy' ), 'share=facebook' );
	}
	...
	<div class="input">
		<label>
			<select name="share_type">
				<option value="default"<?php if ( $this->share_type == 'default' ) echo ' selected="selected"'; ?>><?php _e( 'Default button', 'sharedaddy' ); ?></option>
				<option value="share"<?php if ( $this->share_type == 'share' ) echo ' selected="selected"'; ?>><?php _e( 'Share button', 'sharedaddy' ); ?></option>
				<option value="like"<?php if ( $this->share_type == 'like' ) echo ' selected="selected"'; ?>><?php _e( 'Like button', 'sharedaddy' ); ?></option>
			</select>
		</label>
	</div>
<?php
...
class Share_Print extends Sharing_Source {
	public function get_name() {
		return __( 'Print', 'sharedaddy' );
	}

	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ). ( ( is_single() || is_page() ) ? '#print': '' ), _x( 'Print', 'share to', 'sharedaddy' ), __( 'Click to print', 'sharedaddy' ) );
	}
}

class Share_PressThis extends Sharing_Source {
	public function get_name() {
		return __( 'Press This', 'sharedaddy' );
	}
	...
	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ), _x( 'Press This', 'share to', 'sharedaddy' ), __( 'Click to Press This!', 'sharedaddy' ), 'share=press-this' );
	}
}
...
class Share_Custom extends Sharing_Advanced_Source {
	...
	public function get_display( $post ) {
		$str = $this->get_link( get_permalink( $post->ID ), esc_html( $this->name ), __( 'Click to share', 'sharedaddy' ), 'share='.$this->id );
