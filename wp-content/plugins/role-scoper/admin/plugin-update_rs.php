...
class RS_Updater {
	public static function install( $plugin = '' ) {
		...
		$title = sprintf( _awp('Install %s'), ucwords( str_replace( '-', ' ', $slug ) ) );
...
class RS_Upgrader extends Plugin_Upgrader {
	...
	function download_url ( $url ) {
		...
		if ( ! $url )
			return new WP_Error('http_no_url', _awp('Invalid URL Provided'));

		$request = parse_url($url);
		parse_str($request['query'],$query);
		$tmpfname = wp_tempnam( $query['update'] . ".zip" );
		if ( ! $tmpfname )
			return new WP_Error('http_no_file', _awp('Could not create Temporary file'));
...
class RS_Core_Upgrader extends RS_Upgrader {
	function install_pkg_strings($plugin) {
		if ( $title = ucwords( str_replace( '-', ' ', $plugin ) ) ) {
			$this->strings['no_package'] = _awp('Install package not available.');
			$this->strings['downloading_package'] = sprintf(_awp('Downloading install package from <span class="code">%s</span>&#8230;'),untrailingslashit('http://presspermit.com/'));
			$this->strings['unpack_package'] = _awp('Unpacking the package&#8230;');
			$this->strings['installing_package'] = _awp('Installing the plugin&#8230;');
			$this->strings['process_failed'] = sprintf( _awp('%s install Failed.','pp'), $title );
			$this->strings['process_success'] = sprintf( _awp('%s installed successfully.','pp'), $title );
		}
	}
